<div class="relative">
    <div class="space-y-2">
        ...
        <div class="filament-forms-select-component flex items-center space-x-1 rtl:space-x-reverse group">
            <input type="text" id="airportSelection"
                ...
                placeholder="Search Airports..." wire:model="query" wire:keydown.escape="reset" wire:keydown.tab="reset"
                wire:keydown.arrow-up="decrementHighlight" wire:keydown.arrow-down="incrementHighlight"
                wire:keydown.enter="selectAirport" />

            @if(!empty($query))
            <div class="choices__list choices__list--dropdown is-active block">
                @if(!empty($airports))
                @foreach($airports as $i => $airport)
                <div wire:mouseover="setHighlight({{$i}})" , wire:click="selectAirport"
